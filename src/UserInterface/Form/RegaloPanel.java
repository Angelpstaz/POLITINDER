...
public class RegaloPanel extends JPanel {
    ...
    private void initializeUI() {
        ...
        btnCreate.addActionListener(e -> createRegalo());
        btnUpdate.addActionListener(e -> updateRegalo());
        btnDelete.addActionListener(e -> deleteRegalo());
    ...
    private void loadTableData() {
        try {
            ...
            RegaloBL RegaloBL = new RegaloBL();
            List<RegaloDTO> regalos = RegaloBL.readAll();
    ...
    if (nombre != null && !nombre.isEmpty()) {
        try {
            RegaloDTO nuevoRegalo = new RegaloDTO();
            nuevoRegalo.setNombre(nombre);

            RegaloBL RegaloBL = new RegaloBL();
            boolean creado = RegaloBL.create(nuevoRegalo);
    ...
    if (selectedRow != -1) {
        ...
        if (nuevoNombre != null && !nuevoNombre.isEmpty()) {
            try {
                RegaloDTO regaloActualizado = new RegaloDTO();
                regaloActualizado.setIdRegalo(regaloID);
                regaloActualizado.setNombre(nuevoNombre);

                RegaloBL RegaloBL = new RegaloBL();
                boolean actualizado = RegaloBL.update(regaloActualizado);
    ...
    if (selectedRow != -1) {
        ...
        if (option == JOptionPane.YES_OPTION) {
            ...
            try {
                RegaloBL RegaloBL = new RegaloBL();
                boolean eliminado = RegaloBL.delete(regaloID);
